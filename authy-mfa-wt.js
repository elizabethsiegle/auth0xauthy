  ...
  async.series([
    ...
    function(callback) {
      if (req.method === 'GET') {
        ...
        jwt.verify(context.data.token, new Buffer(context.data.auth0_secret, 'base64'), function(err, decoded){
          if(err){
            res.writeHead(400)
            res.end(err)
          } else {
      ...
      function(err, resp, body){
        ...
        if(body.success){
          ...
          if (typeof err === 'string') {
              return renderOtpView([err], context.body.user_sid);
          }

          if (typeof err === 'object') {
            var errMessage = err.message || err
            renderOtpView([errMessage], context.body.user_sid);
          }
        }
      });
    }
          ...
          <div class="modal-centrix">
            <div class="modal">
              <form onsubmit="showSpinner();" action="" method="POST" enctype="application/x-www-form-urlencoded">
                ...
                <div class="body">
                  <span class="description">Hi <strong><%- user || "" %></strong>, please enter your Authy code.</span>
